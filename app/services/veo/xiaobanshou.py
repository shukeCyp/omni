...
class VeoXiaobanshou(VeoBase):
    ...
    def generate(
        ...
        **kwargs,
        ...
        """生成视频并轮询结果。"""
        del duration  # 小扳手文档未暴露时长参数，当前仅保留接口兼容。
        ...
        if result.video_url and download_dir:
            result.file_path = download_video(result.video_url, download_dir, "veo_xiaobanshou")
        return result
    ...
    def _submit_task(
        ...
        multipart = [
            ("model", (None, self._encode_form_value(model), "text/plain; charset=utf-8")),
            ("prompt", (None, self._encode_form_value(prompt), "text/plain; charset=utf-8")),
            ("size", (None, self._encode_form_value(size), "text/plain; charset=utf-8")),
        ]
        file_handle = None
        try:
            if ref_image_path and os.path.isfile(ref_image_path):
                ...
                file_handle = open(ref_image_path, "rb")
                multipart.append(("input_reference[]", (file_name, file_handle, mime)))
            ...
            response = requests.post(url, headers=headers, files=multipart, timeout=120)
    ...
    def _extract_error_message(response) -> str:
        ...
        return VeoXiaobanshou._extract_error_from_response(body)
